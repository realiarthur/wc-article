...
Все работало отлично и прекрасно смотрелось, но... не все браузеры поддерживают идею расширения встроенных компонентов. Safari cчитает это небезопасным и не поддерживает, это касается и браузеров для iOS (включая Chrome, Firefox и др.). Ходят слухи, что Apple возможно пересмотрит это поведение, но в 13 версии Safari и iOS все остается как есть. Однако, для них есть [полифилы](https://github.com/corpusculejs/custom-builtin-elements), окей, с этим можно как-то жить.



Для меня это было слишком, и до тех пор и если Safari и iOS не начнут поддерживать расширение встроенных элементов я решил не пользоваться этой концепцией.


### Версия 3. Компонент высшего порядка

Я стал оборачивать им компоненты без Shadow DOM, содержащие шаблоны форм. Хотя на его основе можно построить базовый класс (например как в версии 1 или 2) и использовать его в верстке. 
