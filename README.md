...
Я не буду останавливаться на удобствах создания шаблонов, стилизации, передачи атрибутов, привязки данных и подписки на события, условном и цикличном рендере - это подробно описано в [документации](https://lit-html.polymer-project.org/guide/writing-templates). Остановлюсь на том, что возможно не ухватить при беглом взгляде на руководство, но может быть полезно.
...
Кастомные директивы можно также сделать аналогом компонента высшего порядка, если передать ей компонент или шаблон в качестве аргумента и использовать его в `setValue`. При таком подходе необходимо самостоятельно позаботится о реактивности внутри директивы. Пример такого использования можно посмотреть [тут](https://github.com/jmas/lit-redux/blob/master/index.js). 

### Shady render
Если Вы хотите создать расширение для нативного 
