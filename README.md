...
После пары неудачных попыток обернуть функционал формы я решил, что оставлю это за пределами формы и сделаю компонент высшего порядка, чтобы оборачивать им все, что мне понадобиться. Это потребовало небольших изменений кода:
```js
...
Здесь в функции `connectedCallback()` форма принимает конфиг (onSubmit, initialValues, validationSchema, и др.)  либо из аргументов, переданных в `withForm()` либо из самого расширяемого компонента. Это позволяет оборачивать в функционал формы любые классы, а так же строить базовые и использовать их в верстке и в ней же передавать им конфиг. К слову, этим способом можно построить оба базовых класса из первых двух реализаций формы:
...
С другой стророны можно не вводить кастомный элемент для формы, а оборачивать в `withForm()` конечные компоненты содержащие шаблоны форм:
...
```
И расширяемые классы могут как использовать Shadow DOM, так и оставаться открытыми, в зависимости от специфики проекта. 
...
Мне технология кажется перспективной, особенно в части слотов и кастомных элементов, и ее использование вполне может себя оправдать на небольших проектах. Но в текущем состоянии я бы не взял ее на большой проект, над которым работает несколько человек. 

Надеюсь в будущем нативное API станет более удобным и лаконичным.
