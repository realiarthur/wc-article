...
Кастомные директивы можно также использовать как аналог компонента высшего порядка, если передать ей компонент или шаблон в качестве аргумента и использовать его в `setValue`. При таком подходе необходимо самостоятельно позаботится о реактивности внутри директивы. Пример такого использования можно посмотреть [тут](https://github.com/jmas/lit-redux/blob/master/index.js). 
