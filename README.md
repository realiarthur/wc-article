...
С одной стороны мне нужен был функционал тега `<form>` и его класса HTMLFormElement (например, вызов submit при нажатии на Enter), но не хотелось в дополнение к кастомному тегу формы писать его каждый раз в Light DOM (дочерние элементы компонента, вроде children в React). С другой, я не мог использовать слот, чтобы обернуть его содержимое в тег `<form>`, потому что тогда элементы формы фактически располагались бы снаружи (к слову, именно из-за этого стилизовать содержимое слотов можно из внешнего документа) компонента формы, и `<form>` не реагировал бы на их события. 
