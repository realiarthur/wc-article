# Компонентная разработка без фреймворков. Веб-компоненты в реальном проекте.
...
С другой стороны, можно было бы использовать [Svelte](https://github.com/sveltejs/svelte) или, например, [Preact](https://github.com/preactjs/preact). Но попробовать сделать все, используя только встроенные концепции, не используя фреймворки, было слишком заманчивой идеей.


## Будущее уже настало? 
Да, большинство современных браузеров поддерживают веб-компоненты. Для остальных есть [полифилы](https://www.npmjs.com/package/@webcomponents/webcomponentsjs). Примечательно то, что для полифилов есть умный [лоадер](https://www.npmjs.com/package/@webcomponents/webcomponentsjs#using-webcomponents-loaderjs) (~2kB), который загружает только то, что не поддерживается браузером, таким образом ничего не загружая в современные браузеры.
