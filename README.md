...
Если коротко, то из-за отсутствия аналога querySelector, который работает сквозь Shadow DOM, и невозможности автозаполнения сохраненными пользовательскими данными, такими как логин-пароль, адрес, данные кредитных карт. Критичным в моем проекте стал второй недостаток или фича, кому как ближе. Если форма размещена в каком-то компоненте, который использует Shadow DOM автозаполнение перестает работать. То же самое происходит если сам компонент формы или полей ввода использует Shadow DOM. 

>  Конечно, если для передачи шаблона формы используются только слоты, и этот шаблон описан в основном документе, которому он и будет принадлежать, то автозаполнение произойдет, даже при использовании Shadow DOM в компоненте формы. Но кроме этого все компоненты в которые вложена форма и все поля ввода не должны использовать Shadow DOM.
