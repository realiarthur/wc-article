...
```js
// здесь константа IS_LITE_FORM - это булевый атрибут, который имеет каждый элемент кастомной формы
...
Тут все тривиально: рекурсивный поиск элемента с атрибутом, указывающим что это искомая форма. Хотелось отметить что благодаря функции [getRootNode](https://developer.mozilla.org/en-US/docs/Web/API/Node/getRootNode), поиск происходит сквозь все дерево вложенных Shadow DOM - удобная функция при решении таких специфических задач.
